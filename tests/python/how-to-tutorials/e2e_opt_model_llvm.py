...
class MyModule(torch.nn.Module):
    def __init__(self):
        ...
        # O = (58 + 2*1 - 3)/3 + 1 = 20
        self.prevShapeH = (
            IMAGE_SIZE + 2 * self.conv.padding[0] - self.conv.kernel_size[0]
        )
        assert (
            self.prevShapeH % self.conv.stride[0] == 0
        ), "prevShapeH must be divisible by stride"
        self.prevShapeW = (
            IMAGE_SIZE + 2 * self.conv.padding[1] - self.conv.kernel_size[1]
        )
        assert (
        ...
        self.outputsize = self.outputsizeH * self.outputsizeW
        self.linear = torch.nn.Linear(self.outputsize * 1, 10)

    def forward(self, x):
        x = self.conv(x)
        x = x.view(-1, self.outputsize * 1)
        x = self.linear(x)
        return x
