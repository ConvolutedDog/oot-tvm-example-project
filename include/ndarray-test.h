...
void NDArrayTest() {
  ...
  NDArray ndarray = NDArray::Empty(shapetuple, DLDataType({2, 16, 1}), Device({kDLCPU, 0}));
  ...
  dltensor2.shape = const_cast<int64_t*>(shapetuple.data()); // Convert const pointer to non-const.
  dltensor2.strides = nullptr;
  dltensor2.byte_offset = 0;

  // Create another empty NDArray with a different shape.
  NDArray ndarray2 = NDArray::Empty(ShapeTuple({3, 2, 5, 4}), DLDataType({2, 16, 1}), Device({kDLCPU, 0}));
